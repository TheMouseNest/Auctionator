...
if BattlePetToolTip_Show ~= nil then -- not Auctionator.Constants.IsClassic
  local LibBattlePetTooltipLine = LibStub("LibBattlePetTooltipLine")
  hooksecurefunc (_G, "BattlePetToolTip_Show",
    function(speciesID, ...)
      if not addonTable.Config.Get(addonTable.Config.Options.TOOLTIPS_PET) then
        return
      end

      local dbKeys = {"p:" .. speciesID}
      addonTable.Tooltips.AddLines({
        AddDoubleLine = function(_, left, right)
          LibBattlePetTooltipLine:AddDoubleLine(BattlePetTooltip, left, right)
        end
      }, dbKeys, addonTable.Constants.BattlePetCageID, 1)
...
local function GenerateTooltip(tooltip, itemLink, stackCount)
  lastCall = {tooltip, itemLink, stackCount}
  local dbKeys = addonTable.Storage.DBKeyFromLinkFast(itemLink)

  addonTable.Tooltips.AddLines(tooltip, dbKeys, itemLink, stackCount)
end

local TooltipHandlers = {}

-- This is called when mousing over an item in your bags
TooltipHandlers["SetBagItem"] = function(tooltip, bag, slot)
  ...
  if C_Item.DoesItemExist(itemLocation) then
    ...
    GenerateTooltip(tooltip, itemLink, itemCount)
  end
end

-- This is called when mousing over an item in a merchant window (Buyback Pane)
TooltipHandlers["SetBuybackItem"] = function(tooltip, slotIndex)
...
TooltipHandlers["SetMerchantItem"] = function(tooltip, index)
...
TooltipHandlers["SetInventoryItem"] = function(tooltip, unit, slot)
  local itemLink = GetInventoryItemLink(unit, slot)
  local itemCount = GetInventoryItemCount(unit, slot)

  GenerateTooltip(tooltip, itemLink, itemCount ~= 0 and itemCount or 1)
end
...
-- price is difficult.
TooltipHandlers["SetGuildBankItem"] = function(tooltip, tab, slot)
...
if GameTooltip.SetRecipeResultItem then -- Dragonflight onwards
  TooltipHandlers["SetRecipeResultItem"] = function(tooltip, recipeID, reagents, allocations, recipeLevel, qualityID)
    ...
    if outputLink then
      ...
      GenerateTooltip(tooltip, outputLink, recipeSchematic.quantityMin)
    end
  end
end

if GameTooltip.SetItemByGUID then
  TooltipHandlers["SetItemByGUID"] = function(tooltip, guid)
    local itemLink = C_Item.GetItemLinkByGUID(guid)

    if itemLink then
      GenerateTooltip(tooltip, itemLink, 1)
    end
  end
end

if GameTooltip.SetTradeSkillItem then -- Classic
  TooltipHandlers["SetTradeSkillItem"] = function(tooltip, recipeIndex, reagentIndex)
...
if GameTooltip.SetCraftItem then -- TBC classic and earlier
  TooltipHandlers["SetCraftItem"] = function(tooltip, recipeIndex, reagentIndex)
    local itemLink, itemCount
    if reagentIndex ~= nil then
...
TooltipHandlers["SetLootItem"] = function (tooltip, slot)
...
TooltipHandlers["SetLootRollItem"] = function (tooltip, slot)
...
TooltipHandlers["SetQuestItem"] = function (tooltip, type, index)
...
TooltipHandlers["SetQuestLogItem"] = function (tooltip, type, index)
...
TooltipHandlers["SetSendMailItem"] = function (tooltip, id)
...
TooltipHandlers["SetInboxItem"] = function(tooltip, index, attachIndex)
...
TooltipHandlers["SetTradePlayerItem"] = function (tooltip, id)
...
TooltipHandlers["SetTradeTargetItem"] = function (tooltip, id)
...
if GameTooltip.SetAuctionItem then
  TooltipHandlers["SetAuctionItem"] = function(tooltip, viewType, index)
...
if GameTooltip.SetItemKey then
  TooltipHandlers["SetItemKey"] = function(tooltip, itemID, itemLevel, itemSuffix)
    local itemLink
...
TooltipHandlers["SetItemByID"] = function (tooltip, itemID)
  if not itemID then
    return
  end
...
if TooltipDataProcessor and C_TooltipInfo then
  ...
  GameTooltip:HooKScript("OnTooltipClea")
end
